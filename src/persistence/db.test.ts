...
import { validateUpload, saveVideo, loadVideo, saveRecording, listRecordings, updateRecording } from "./db";
...
describe("updateRecording", () => {
    ...
    it("aktualisiert nur die übergebenen Felder, Rest bleibt erhalten", async () => {
        ...
        const found = (await listRecordings()).find((r) => r.recordingId === recordingId);
    ...
    it("aktualisiert Rating und Kommentar zusammen", async () => {
        ...
        await updateRecording(recordingId, { rating: 5, comment: "Sehr flüssig vorgelesen!" });
    ...
    it("wirft einen Fehler bei nicht existierender recordingId", async () => {
        await expect(updateRecording("does-not-exist", { status: "normal" })).rejects.toThrow();
    });
});
