...
describe("findNextCuePoint", () => {
  ...
  it("springt zum nächsten Cue-Point, wenn currentTime vor dem ersten liegt", () => {
    expect(findNextCuePoint(cues, -5)).toBe(0);
  });
});
...
describe("findPreviousCuePoint", () => {
  ...
  it("bleibt beim letzten Cue-Point, wenn currentTime nach dem letzten liegt", () => {
    expect(findPreviousCuePoint(cues, 200)).toBe(75);
  });
});
